angular.module('mcrrcApp.results').controller('ReportController', ['$scope', '$stateParams', '$analytics', '$filter', 'AuthService', 'ResultsService', function($scope, $stateParams, $analytics, $filter, AuthService, ResultsService) {
    ...
    $scope.getResults = function() {
        ...
        ResultsService.getRacesInfos({
            ...
            "sort": '-racedate racename'
        }).then(function(races) {
            $scope.report = "";
            races.forEach(function(race) {
                $scope.report += race.racename + " -- " + $filter('date')(race.racedate, "yyyy-MM-dd", 'UTC') + "\n";
        ...
        ResultsService.getRacesInfos({
            ...
            races.forEach(function(race) {
                ...
                race.results.forEach(function(result) {
                    ...
                    members = members.slice(0, -3);
                    $scope.reportHTLM += '<tr><td style="font-weight:bold;">' + members + '</td>' + $filter('rankTooltipTd')(result.ranking) + '<td style="text-align: center;"><span style="cursor:pointer;" title="pace: ' + $filter('resultToPace')(result,race) + '">' + $filter('secondsToTimeString')(result.time) + '</span></td></tr>';
                });
            });
