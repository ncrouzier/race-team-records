...
app.directive('resultPace', function() {
	var reg ='<span class="resultPace" ng-if="race.racetype.name !== \'Swim\' && race.racetype.name !== \'Cycling\'" ><span class="hoverhand" ng-show="result.agegrade !== undefined && result.agegrade>=90" uib-tooltip-html=\'"<span>Age Grade: {{result.agegrade}}%</span><br><span>World Class</span>" | unsafe \'>{{result | resultToPace:race}} <i  class="ageworld fa fa-star"></i><br><small class="resultPaceTxt">min/mi</small></span><span class="hoverhand" ng-show="result.agegrade !== undefined && result.agegrade>=80 && result.agegrade<90" uib-tooltip-html=\'"<span>Age Grade: {{result.agegrade}}%</span><br><span>National Class</span>" | unsafe\'>{{result | resultToPace:race}} <i class="agenational fa fa-star"></i><br><small class="resultPaceTxt">min/mi</small></span><span class="hoverhand" ng-show="result.agegrade !== undefined && result.agegrade>=70 && result.agegrade<80" uib-tooltip-html=\'"<span>Age Grade: {{result.agegrade}}%</span><br><span>Regional Class</span>" | unsafe\'>{{result | resultToPace:race}} <i class="ageregional fa fa-star"></i><br><small class="resultPaceTxt">min/mi</small></span><span class="hoverhand" ng-show="result.agegrade !== undefined && result.agegrade<70" uib-tooltip-html=\'"<span>Age Grade: {{result.agegrade}}%</span>" | unsafe\'>{{result | resultToPace:race}}<br><small class="resultPaceTxt">min/mi</small></span><span ng-show="result.agegrade === undefined">{{result | resultToPace:race}}<br><small class="resultPaceTxt">min/mi</small></span></span>';
	var swim = '<span class="resultPace" ng-if="race.racetype.name === \'Swim\'"><span>{{result | resultToSwimPace}}</span><br><small class="resultPaceTxt">/100m</small></span>';
	var bike = '<span class="resultPace" ng-if="race.racetype.name === \'Cycling\'"><span>{{result | resultToBikePace}}</span><br><small class="resultPaceTxt">mph</small></span>';
    return {
        scope: {
            race:'=race',
            result:'=result'
