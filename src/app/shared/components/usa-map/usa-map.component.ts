...
export class UsaMapComponent implements AfterViewInit, OnChanges, OnDestroy {
  @Input() stateData: [string, number][] = [];
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.initialized && changes['stateData'] && this.stateData && this.stateData.length > 0) {
      this.renderMap();
    }
  }
  ...
  private renderMap(): void {
    ...
    const onlyValues = this.stateData.map(item => item[1]);
